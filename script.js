const portfolio = {
  ...
  highlights: [
    { value: "25+", label: "REST API endpoints shipped" },
    { value: "150+", label: "production users supported" },
    { value: "30%", label: "workflow overhead reduced" },
    { value: "4.0", label: "graduate GPA at ASU" }
  ],
  featuredStack: ["Java", "Spring Boot", "PostgreSQL", "AWS", "Docker", "JWT"],
  ...
  projects: [
    {
      ...
      links: [
        {
          label: "Play Store",
          url: "https://play.google.com/store/apps/details?id=com.sitatci.travart"
        }
      ]
    },
    {
      ...
      links: [
        {
          label: "GitHub",
          url: "https://github.com/keshavsoni01/Data-Management-using-Spring-boot-with-JWT"
        }
      ]
    },
    {
      ...
      links: [
        {
          label: "GitHub",
          url: "https://github.com/keshavsoni01/Accident-detection-system-using-Flask"
        }
      ]
    }
  ],
  ...
  publications: [
    {
      ...
      url: "https://ieeexplore.ieee.org/document/10456108"
    },
    {
      ...
      url: "https://link.springer.com/chapter/10.1007/978-981-99-6547-2_32"
    }
  ],
  certifications: [
    {
      ...
      summary: "Validates cloud architecture fundamentals across scalable, reliable, and cost-aware AWS solutions.",
...
const guiHighlights = document.getElementById("gui-highlights");
const guiFacts = document.getElementById("gui-facts");
const guiStackStrip = document.getElementById("gui-stack-strip");
...
const guiCertificationsGrid = document.getElementById("gui-certifications-grid");
...
const MODE_HASHES = new Set(["#choice", "#terminal", "#gui"]);
...
function resourceLinks(items = []) {
  const links = items.filter((item) => item && item.url);
  ...
  return links
    .map((item) => `<p><span class="muted">${escapeHtml(item.label || "Link")}:</span> ${link(item.url, item.url)}</p>`)
    .join("");
}
...
function getCurrentAnchorTarget() {
  const hash = window.location.hash;

  if (!hash || MODE_HASHES.has(hash)) {
    return null;
  }
...
function setActiveMode(mode) {
  ...
  if (activeMode === "gui") {
    ...
    const hashTarget = getCurrentAnchorTarget();
    ...
    <div class="grid">
      ${portfolio.publications.map((paper) => `
        <article class="item">
          ...
          ${resourceLinks([{ label: "Publication", url: paper.url }])}
        </article>
      `).join("")}
    </div>
  `);
...
function certificationsCommand() {
  return block(`
    <p class="kicker">certifications</p>
    <div class="grid">
      ${portfolio.certifications.map((certification) => `
        <article class="item">
          <div class="item-name">${escapeHtml(certification.title)}</div>
          <div class="item-copy">${escapeHtml(certification.issuer)}</div>
          <p>${escapeHtml(certification.summary)}</p>
          ${resourceLinks([{ label: "Details", url: certification.url }])}
        </article>
      `).join("")}
    </div>
  `);
...
function renderGuiPortfolio() {
  ...
  guiHighlights.innerHTML = portfolio.highlights.map((item) => `
  ...
  guiStackStrip.innerHTML = portfolio.featuredStack.map((item) => `<span>${escapeHtml(item)}</span>`).join("");
  ...
  guiPublicationsGrid.innerHTML = portfolio.publications.map((paper) => `
    <article class="gui-card">
      ...
      ${guiActions([{ label: "Read publication", url: paper.url }])}
    </article>
  `).join("");

  guiCertificationsGrid.innerHTML = portfolio.certifications.map((certification) => `
    <article class="gui-card">
      <p class="card-kicker">${escapeHtml(certification.issuer)}</p>
      <h3>${escapeHtml(certification.title)}</h3>
      <p>${escapeHtml(certification.summary)}</p>
      ${guiActions([{ label: "Certification details", url: certification.url }])}
    </article>
  `).join("");
...
function getOpenTargets() {
  const projectLinks = portfolio.projects.reduce((targets, project, index) => {
    const url = project.links && project.links[0] ? project.links[0].url : "";
    const slug = project.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

    if (url) {
      targets[`project${index + 1}`] = url;
      targets[slug] = url;
    }
  ...
  return {
    ...
    travart: projectLinks.travart,
    data: projectLinks["data-management-system"],
    accident: projectLinks["accident-detection-system"],
    paper1: portfolio.publications[0] && portfolio.publications[0].url,
    ieee: portfolio.publications[0] && portfolio.publications[0].url,
    paper2: portfolio.publications[1] && portfolio.publications[1].url,
    springer: portfolio.publications[1] && portfolio.publications[1].url,
    aws: portfolio.certifications[0] && portfolio.certifications[0].url,
    certification: portfolio.certifications[0] && portfolio.certifications[0].url,
    ...projectLinks
  };
}
...
function openCommand(args) {
  ...
  if (!target || !(target in targets)) {
    return message("open", [
      "Targets: github, linkedin, email, resume, travart, data, accident, paper1, paper2, aws",
      "Examples: open travart, open paper2"
    ]);
  }
...
const commands = {
  ...
  open: {
    description: "Open a profile, project, publication, or certification link.",
    usage: "open github|linkedin|email|resume|travart|data|accident|paper1|paper2|aws",
    run: openCommand
  },
  copy: {
...
const aliases = {
  ...
  email: "open email",
  travart: "open travart",
  data: "open data",
  accident: "open accident",
  ieee: "open ieee",
  springer: "open springer",
  aws: "open aws",
  hire: "contact"
};
